...
def multiple_matplot(df_c, n1 : int, n2 : int):
    ...
    ax[0].plot(x1, y1)
    ax[1].plot(x2, y2)
    ax[0].set_title(f"patient {id[0]}")
    ax[1].set_title(f"patient {id[1]}")
...
def add_event(df_c, df_g):
    ...
    for i in range(0, len(id_unique)):
        ...
        df = pd.DataFrame()
        print("len", len(sub_event["id"]))
        for j in range(0, len(sub_event["id"])):
            print("len", len(sub_event["id"]))
            if not sub_df_c.empty:
                print("entrer dans al boucle j")
                sub_df_c.loc[sub_df_c['ts_machine'] >= sub_event.loc[j, "ts"], 'event'] = 1
                df = pd.concat(df, sub_df_c)
                print("apres modif", sub_df_c)
            else:
                print('empty')
            #& sub_df_c['ts_machine'] >= (sub_event["ts"][j] + sub_event["len"][j])
